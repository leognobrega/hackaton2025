...
(function(){
  ...
  const darkPref = sessionStorage.getItem('gmu_dark') === '1';
  if(!darkPref){ document.documentElement.setAttribute('data-bs-theme','light'); }
  ...
  function renderOpenOccurrences(){
    ...
    if(!list) return;
    list.innerHTML = '';
    // assume ocorrencias[0] is newest — show most recent first
    const open = ocorrencias.filter(o=> !(o.status && o.status.toLowerCase().includes('fechado')));
    for(const o of open){
      const li = document.createElement('li');
      li.innerHTML = `<strong>#${o.id}</strong> ${o.tipo} <span class="small text-muted">- ${o.risco || 'Baixo'}</span> <div class="small text-muted">${o.status||''}</div>`;
      list.appendChild(li);
    }
  }
  ...
  if(typeof oldSim === 'function'){
    window.simularOcorrencia = function(){ oldSim(); renderOcorrencias(); renderCompact(); refreshKPIs(); updateOccChart(); renderOpenOccurrences(); }
  }
  ...
  document.getElementById('toggleTheme').addEventListener('click', ()=>{
    ...
    sessionStorage.setItem('gmu_dark', next==='dark' ? '1' : '0');
  });
